<div>
    <div class="my-5 max-w-screen-xl mx-auto px-4 lg:px-8">
            ...
            <div class="lg:w-1/4 xl:w-1/2 pt-5 md:pt-14 px-5 sm:px-0 flex flex-row-reverse">
                <div class="py-8 w-full md:w-4/6">
                    <div class="rounded-xl bg-white">
                        ...
                        <form wire:submit.prevent='convert' class="p-3">
                            <div class="flex flex-col md:flex-row space-x-0 md:space-x-2">
                                <div>
                                    <x-select :label="__('From')" :placeholder="__('From')" wire:model="from">
                                        @foreach ($currencies as $currency)
                                        <x-select.option :label="Str::upper($currency['code'])"
                                            :value="Str::upper($currency['code'])" />
                                        @endforeach
                                    </x-select>
                                </div>
                                <div>
                                    <x-select :label="__('To')" :placeholder="__('To')" wire:model="to">
                                        @foreach ($currencies as $currency)
                                        <x-select.option :label="Str::upper($currency['code'])"
                                            :value="Str::upper($currency['code'])" />
                                        @endforeach
                            ...
                            <div
                                ...
                                <input type="number" min="0" wire:model.defer="value"
                                    class="px-3 text-gray-700 text-sm border rounded flex-grow border-gray-700 focus:outline-none py-1 focus:border-yellow-500"
                                    placeholder="Amount" />
                                <x-button type="submit" lg
                            ...
                            <div class="flex justify-center p-2">
                                <span class="text-3xl text-gray-800 rounded-full  py-2 px-4">{{$result}}</span>
                            </div>
                        </form>
                    </div>
                </div>
        ...
        <div class="mt-20 max-w-screen-sm mx-auto lg:mx-0 lg:max-w-none grid lg:grid-cols-3 gap-10 lg:gap-8 text-sm">
            <div class="flex space-x-6">
                <div class="w-10 h-10 rounded bg-gray-800 text-center flex-none text-yellow-400">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mt-2 ml-2 " fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" />
                    </svg>
